...
class MLService:
    ...
    def _compute_feature_contributions(self, stage: str, input_dict: Dict[str, Any], score: float) -> Dict[str, Any]:
        """Provides intuitive feature influence insights for student dashboard."""
        if stage == "university":
            att = float(input_dict.get("Attendance_Pct", input_dict.get("attendance_pct", 85.0)))
            study = float(input_dict.get("Study_Hours_Per_Day", input_dict.get("study_hours_per_day", 4.0)))
            prev = float(input_dict.get("Previous_CGPA", input_dict.get("previous_cgpa", 3.4)))
            return {
                "top_positive_factors": [
                    f"Strong baseline GPA ({prev:.2f}) accounts for major baseline predictive weight",
                    f"High attendance rate ({att:.1f}%) positively reinforces academic consistency",
                    f"Daily study routine ({study:.1f} hrs/day) solidifies conceptual retention",
                ],
                "growth_areas": [
            ...
            return {
                "top_positive_factors": [
                    "Consistent performance across SSC-I and SSC-II matric milestones",
                    "Steady daily study discipline",
                ],
                "growth_areas": [
        ...
        return {
            "top_positive_factors": ["High core engagement across assessment periods"],
            "growth_areas": ["Target regular revision before term examinations"],
        }
